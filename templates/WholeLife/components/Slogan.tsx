...
export default function Slogan({ onOpenPrivacy }: SloganProps) {
  ...
  const consultOtpInputRefLocal = useRef<HTMLInputElement>(null);
  const handleConsultSendOTP = async () => {
    setConsultOtpTimer(OTP_TIMER_DURATION);
    setConsultOtpResendAvailable(false);
    await handlePostOTP();
    setTimeout(() => {
      consultOtpInputRefLocal.current?.focus();
      consultOtpInputRefLocal.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }, 300);
  };
  ...
  const calculateInsuranceValues = () => {
    ...
    const total =
      !isNaN(amount) && !isNaN(months) && amount > 0 && months > 0 ? amount * months : 0;
