...
export async function POST(request: NextRequest) {
  try {
    ...
    const { customerName, gender, age, paymentPeriod, monthlyPayment, productType, mode } =
      await request.json();
    ...
    if (mode === 'eligibility') {
      if (!gender || !age) {
        ...
        return NextResponse.json({ error: '성별과 연령이 필요합니다.' }, { status: 400 });
      }
    } else {
      if (!customerName || !gender || !age || !paymentPeriod || !monthlyPayment) {
        console.log('[API] 필수 입력값 누락:', {
          customerName,
          gender,
          age,
          paymentPeriod,
        ...
        return NextResponse.json({ error: '모든 필수 입력값이 필요합니다.' }, { status: 400 });
      }
    }
    ...
    if (resolvedProductType === 'happy-dream') {
      candidatePaths = [
        path.join(process.cwd(), 'public', 'resources', 'data', 'kdb-dream-data.xlsx'),
      ];
    ...
    if (!excelFilePath) {
      return NextResponse.json({ error: '엑셀 파일을 찾을 수 없습니다.' }, { status: 500 });
    }
    ...
    try {
      ...
      return NextResponse.json({ error: '엑셀 파일을 읽을 수 없습니다.' }, { status: 500 });
    }
    ...
    if (!worksheet) {
      return NextResponse.json({ error: `시트를 찾을 수 없습니다.` }, { status: 500 });
    }
    ...
    if (headerRowIndex === -1) {
      ...
      return NextResponse.json({ error: '필수 컬럼을 찾을 수 없습니다.' }, { status: 500 });
    }
    ...
    if (mode === 'eligibility') {
      ...
      for (const p of periodsToCheck) {
        ...
        for (let i = headerRowIndex + 1; i < jsonData.length; i++) {
          ...
          const rowGender = rawGender.startsWith('남')
            ? '남'
            : rawGender.startsWith('여')
              ? '여'
              : rawGender;
    ...
    if (!matchedRow) {
      ...
      return NextResponse.json({ error: '일치하는 데이터를 찾을 수 없습니다.' }, { status: 404 });
    }
    ...
    const pensionStartAge =
      pensionStartAgeIndex !== -1 ? parseNumber(matchedRow[pensionStartAgeIndex]) : 0;
    const monthlyPension =
      monthlyPensionIndex !== -1 ? parseNumber(matchedRow[monthlyPensionIndex]) : 0;
    ...
    const guaranteedAmount =
      guaranteedAmountIndex !== -1 ? parseNumber(matchedRow[guaranteedAmountIndex]) : 0;
    const totalUntil100 =
      totalUntil100Index !== -1 ? parseNumber(matchedRow[totalUntil100Index]) : 0;
