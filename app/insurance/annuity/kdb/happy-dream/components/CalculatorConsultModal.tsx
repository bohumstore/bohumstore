...
const INSURANCE_PRODUCT_ID = 2; // KDB 더!행복드림변액연금보험
...
export default function CalculatorConsultModal({ isOpen, onClose, type }: CalculatorConsultModalProps) {
  ...
  // ── Pension calc (API-based, identical to existing Happy Dream Slogan logic) ──
    ...
    const fetchStartAge = async () => {
      try {
        ...
        const resp = await fetch('/api/calculate-pension/', {
          ...
          body: JSON.stringify({ customerName: 'temp', gender, age: Number(insuranceAge), paymentPeriod: years, monthlyPayment: mp, productType: 'happy-dream' }),
        });
        if (resp.ok) {
          const data = await resp.json();
          if (data.data?.pensionStartAge) { setCurrentPensionStartAge(data.data.pensionStartAge); return; }
        }
        setCurrentPensionStartAge(getPensionStartAge(Number(insuranceAge), paymentPeriod));
      } catch { setCurrentPensionStartAge(getPensionStartAge(Number(insuranceAge), paymentPeriod)); }
    };
    fetchStartAge();
  ...
  const calculatePensionAmount = async (age: number, pp: string, pa: string, g: string) => {
    if (!age || !pp || !pa || !g) return { monthly: 0, performance: 0, guaranteed: 0, totalUntil100: 0, pensionStartAge: 0, notice: '' };
    try {
      let mp = 0;
      ...
      const res = await fetch('/api/calculate-pension/', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ customerName: name, gender: g, age, paymentPeriod: py, monthlyPayment: mp, productType: 'happy-dream' }),
      });
      if (!res.ok) throw new Error('fail');
      const result = await res.json();
      if (result.success) return {
        monthly: result.data.monthlyPension, performance: result.data.performancePension || 0,
        guaranteed: result.data.guaranteedAmount, totalUntil100: result.data.totalUntil100,
        pensionStartAge: result.data.pensionStartAge, notice: result.data.notice,
      };
      throw new Error('fail');
    } catch { return { monthly: 0, performance: 0, guaranteed: 0, totalUntil100: 0, pensionStartAge: 0, notice: '' }; }
  };

  // ── Supabase save (identical to existing Happy Dream Slogan logic) ──
  ...
  const handleVerifyOTP = async () => {
    ...
    try {
      ...
      if (paymentPeriod && paymentAmount) {
        calc = await calculatePensionAmount(Number(insuranceAge), paymentPeriod, paymentAmount, gender);
        setPensionAmounts(calc);
      }
  ...
  const renderStep1 = () => (
    <div className="space-y-5">
      <StepHeader 
        title={type === 'calculate' ? '연금액 계산 정보 입력' : '상담 신청 정보 입력'} 
          ...
          <CustomSelect
            ...
            options={['10년', '15년', '20년'].map(p => {
              ...
              return {
                value: p,
                disabled: isDisabled,
                label: isDisabled ? `${p} (가입불가)` : p === '10년' ? (
                  <span className="flex items-center gap-1">
                    10년 <span className="px-1.5 py-0.5 rounded text-[10px] font-bold bg-brand-primary-soft text-brand-primary leading-none">추천</span>
                  </span>
                ) : p
              };
            })}
            ...
            <div className="bg-page-bg rounded-lg px-4 py-3 body-m mb-2 flex flex-col gap-1">
              <div className="flex items-center">
                ...
                <span className="font-bold flex gap-1 items-center text-brand-primary">
                  <span className="blur-sm select-none">약 742,000</span>
                  원
                </span>
              </div>
            </div>
          ...
          >
            <div className="space-y-2 select-none pointer-events-none bg-page-bg rounded-xl p-4">
              {[
                ...
                { label: '상품명',                       value: '더!행복드림변액연금보험' },
                { label: '납입기간/월보험료',              value: `${paymentPeriod} / ${paymentAmount}` },
                { label: '20년 보증기간 연금액',           value: '약 742,000 원' },
                { label: '100세까지 생존 시 총 받는 금액', value: '약 152,000,000 원' },
              ].map(item => (
        ...
        <div className="rounded-xl bg-page-bg p-4">
          ...
          <div className="space-y-2">
            {[
              ...
              { label: '상품명', value: '더!행복드림변액연금보험', color: 'text-brand-primary' },
