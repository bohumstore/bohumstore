...
export default function CarerInsurancePage() {
  ...
  // 햄버거 메뉴 및 윈도우 이벤트 감지 (ProductDetailTemplate 로직 복제)
  useEffect(() => {
    const handleMenuChange = (e: any) => setIsHeaderMenuOpen(e.detail.isOpen);
    window.addEventListener('headerMenuChange', handleMenuChange as EventListener);
    
    const clickBtn = (selector: string) => document.querySelector<HTMLElement>(selector)?.click();
    ...
    return () => {
      window.removeEventListener('headerMenuChange', handleMenuChange as EventListener);
  ...
  const showFloating = !isHeroActionsVisible && !isModalOpen && !showNotice && !isInputFocused && !isHeaderMenuOpen;
  ...
  return (
    ...
    >
      ...
      <SloganSection
        ...
        illustrationSrc="/svgs/slogan/carer/slogan-carer-hand-heart.svg"
        illustrationAlt="간병인보험 일러스트"
        checkItems={[
          <>하루 최대 <span className="font-bold">20만원</span> 간병인 사용료 지급</>,
          <>월 <span className="font-bold">2~5만원</span>으로 간병비 걱정 해결</>,
          <>가족간병시에도 <span className="font-bold">보험금 지급</span></>,
        ]}
        cardContent={
          <SloganCardView
            title="간병인 비용, 얼마나 드는지 아시나요?"
            bottomInfoText="간병인 보험으로 하루 최대 20만원 보장"
            bottomInfoIcon="/svgs/slogan/carer/slogan-carer-doctor.svg"
      ...
      <div className="w-full bg-white md:px-6 md:py-12 lg:px-8 lg:py-12">
