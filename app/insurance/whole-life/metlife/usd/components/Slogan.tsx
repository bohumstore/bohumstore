...
export default function Slogan({ onOpenPrivacy, onModalStateChange }: SloganProps) {
  ...
  const [consultType, setConsultType] = useState('백만인을위한달러종신보험Plus');
  const [consultTime, setConsultTime] = useState('아무때나');
  const consultTypeOptions = ['백만인을위한달러종신보험Plus'];
          ...
          <div className="flex-1 flex justify-center lg:justify-end w-full max-w-md sm:max-w-lg lg:max-w-none">
            <div id="calculator-box" className="w-full max-w-md sm:max-w-lg bg-white rounded-2xl shadow-2xl p-5 sm:p-6 md:p-7 relative flex flex-col">
              ...
              <div className="mb-5 sm:mb-6">
                ...
                <p className="text-gray-700 text-xs sm:text-sm ml-10">간단한 정보 입력으로 예상 달러환급액을 확인하세요</p>
              </div>
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {['5년납', '7년납', '10년납'].map((period) => (
                      <label key={period} className="relative cursor-pointer">
                        ...
                        <div className={`w-full text-center py-2.5 text-sm border-2 rounded-lg transition-all ${paymentPeriod === period ? 'border-[#00529b] bg-[#00529b]/5 text-[#00529b] font-bold' : 'border-gray-200 hover:border-gray-300'}`}>
                          {period}
                        </div>
                      </label>
                    ))}
                    ...
                    {[
                      ...
                      <label key={item.krw} className="cursor-pointer">
                        ...
                        <div className={`w-full text-center py-2 text-sm border-2 rounded-lg transition-all ${paymentAmount === item.krw ? 'border-[#00529b] bg-[#00529b]/5 text-[#00529b] font-bold' : 'border-gray-200 hover:border-gray-300'}`}>
                          <div>{item.krw}</div>
                          <div className={`text-xs ${paymentAmount === item.krw ? 'text-[#00529b]/70' : 'text-gray-600'}`}>약 {item.usd}$</div>
                        </div>
                      </label>
                    ))}
                  </div>
                ...
                <div className="bg-white p-1.5 sm:p-2 rounded border border-gray-200">
                  <div className="flex justify-between items-center text-sm">
                    ...
                    <span className="font-bold">
                      ...
                      <span className="text-gray-600 text-xs ml-1 font-normal">(약 {total ? `${Math.round(total / BASE_EXCHANGE_RATE).toLocaleString()}$` : '-'})</span>
                    </span>
                  </div>
                </div>
                {/* 10년 시점 환급률 */}
                ...
                <div className="bg-white p-1.5 sm:p-2 rounded border border-gray-200">
                  <div className="flex justify-between items-center text-sm">
                    ...
                    <span className="font-bold">
                      ...
                      <span className="text-gray-600 text-xs ml-1 font-normal">(약 {total ? `${Math.round(total * interestRate / BASE_EXCHANGE_RATE).toLocaleString()}$` : '-'})</span>
                    </span>
                  </div>
                </div>
                <div className="bg-white p-1.5 sm:p-2 rounded border border-gray-200">
                  <div className="flex justify-between items-center text-sm">
                    ...
                    <span className="font-bold">
                      ...
                      <span className="text-gray-600 text-xs ml-1 font-normal">(약 {total ? `${Math.round(total * rate / BASE_EXCHANGE_RATE).toLocaleString()}$` : '-'})</span>
                    </span>
                  </div>
                </div>
