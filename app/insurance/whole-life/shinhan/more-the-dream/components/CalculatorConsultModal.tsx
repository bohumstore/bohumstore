...
const INSURANCE_COMPANY_ID = INSURANCE_COMPANIES.SHINHAN_LIFE;
const INSURANCE_PRODUCT_ID = INSURANCE_PRODUCTS.SHINHAN_MORE_THE_DREAM;
...
export default function CalculatorConsultModal({ isOpen, onClose, type }: CalculatorConsultModalProps) {
  ...
  let rate = 1.227, interestRate = 0.227;
  if (paymentPeriod.includes('5')) { rate = 1.227; interestRate = 0.227; }
  else if (paymentPeriod.includes('7')) { rate = 1.195; interestRate = 0.195; }
  else if (paymentPeriod.includes('10')) { rate = 1.15; interestRate = 0.15; }
        ...
        <div className="space-y-5">
          ...
          <StepSection title="··· 상세 정보 보기" titleClassName="heading-5 text-text-primary mb-3 flex items-center">
            <div className="space-y-2 select-none pointer-events-none">
              {[
                { label: '보험사', value: '신한라이프', color: 'text-brand-primary' },
                { label: '상품명', value: '모아더드림Plus종신보험', color: 'text-brand-primary' },
                ...
                { label: '10년 시점 환급률', value: rate ? `${(rate * 100).toFixed(1)}%` : '000.0%', color: 'text-category-purple' },
        ...
        <div className="rounded-xl bg-page-bg p-4">
          ...
          <div className="space-y-2">
            {[
              ...
              { label: '10년 시점 환급률', value: rate ? `${(rate * 100).toFixed(1)} %` : '-', color: 'text-category-purple' },
              { label: '10년 시점 이자', value: `${interestValue} 원`, color: 'text-category-health' },
              { label: '10년 시점 예상 해약환급금', value: `${refundValue} 원`, color: 'text-status-red' },
            ].map(item => (
