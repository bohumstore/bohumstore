...
function toMainProduct(p: UnifiedProduct): MainProduct {
  ...
  return {
    ...
    badge: m.badge,
    category: p.category === 'annuity' ? (
      p.id === 'kb-triple-level-up' ? '연금보험' :
      p.id === 'kdb-happy-dream' ? '변액연금' :
      p.id === 'ibk-lifetime' ? '변액연금' :
      p.id === 'kdb-happy-plus' ? '연금보험' :
      '연금보험'
    ) : p.category === 'whole-life' ? (
      p.id === 'metlife-usd' ? '달러종신보험' :
      '종신보험'
    ) : p.category,
...
export const slogans: Slogan[] = [
  consultSlogan,
  ...getSloganProducts().map(toSlogan),
];
